...
type ReqType = "help" | "pain" | "water" | "food" | "bathroom" | "company" | "other";
...
function PatientMode() {
  ...
  async function sendRequest(type: ReqType, label: string, msg?: string, painLevel?: number) {
    if (!circle || !user) return;
    await supabase.from("patient_requests").insert({
      circle_id: circle.id, patient_id: user.id, type, message: msg ?? null, pain_level: painLevel ?? null,
    });
    toast(label);
  }

  async function logMood(level: number) {
    if (!circle || !user) return;
    setMood(level);
    await supabase.from("patient_moods").insert({
      circle_id: circle.id, patient_id: user.id, mood: level, pain_level: pain,
    });
    toast("Mood saved");
  }
  ...
  async function sendMessage() {
    ...
    await sendRequest("other", "Your message was sent.", message);
    setMessage("");
  }

  const moods = [
    { label: "Great", level: 5, icon: Laugh, color: "text-success" },
    { label: "Good", level: 4, icon: Smile, color: "text-success" },
    { label: "Okay", level: 3, icon: Meh, color: "text-warning-foreground" },
    { label: "Low", level: 2, icon: Annoyed, color: "text-accent-foreground" },
    { label: "Sad", level: 1, icon: Frown, color: "text-destructive" },
  ];
    ...
    <div className="min-h-dvh bg-gradient-to-b from-primary-soft via-background to-background">
      <header className="px-5 sm:px-8 py-5 flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <div className="text-lg font-bold">{profile?.full_name ?? "You"}</div>
        </div>
      </header>

      <main className="mx-auto max-w-3xl px-5 sm:px-8 pb-16">
        <div className="text-center mt-4">
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight">How are you?</h1>
          <p className="mt-3 text-lg sm:text-xl text-muted-foreground">Tap a big button. Your circle will know right away.</p>
        </div>

        <button
          onClick={() => sendRequest("help", "Your circle has been alerted — help is on the way.")}
        ...
        <div className="mt-6 grid sm:grid-cols-3 gap-4">
          <BigButton icon={HandHelping} label="I Need Help" tone="primary" onClick={() => sendRequest("help", "We told your circle you need help.")} />
          <BigButton icon={HeartPulse} label="I Am In Pain" tone="warning" onClick={() => sendRequest("pain", "Your pain message was sent.", undefined, pain)} />
          <BigButton icon={Droplet} label="I Need Water" tone="info" onClick={() => sendRequest("water", "Someone will bring you water.")} />
        </div>
        ...
        <section className="mt-6 rounded-3xl bg-card border border-border p-6 sm:p-8">
          ...
          <input type="range" min={0} max={10} value={pain} onChange={(e) => setPain(Number(e.target.value))} className="w-full mt-5 h-3 rounded-full appearance-none bg-gradient-to-r from-success via-warning to-destructive accent-primary" aria-label="Pain level" />
          <div className="mt-2 grid grid-cols-11 text-xs text-muted-foreground">
            {Array.from({ length: 11 }, (_, i) => <span key={i} className="text-center">{i}</span>)}
          </div>
        </section>
        ...
        <section className="mt-6 rounded-3xl bg-card border border-border p-6 sm:p-8">
          ...
          <textarea
            ...
            placeholder="I'd like a cup of tea please…" rows={3}
          ...
          />
          <button onClick={sendMessage} className="mt-4 inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl bg-primary text-primary-foreground text-lg font-bold hover:opacity-90">
...
function BigButton({ icon: Icon, label, tone, onClick }: { icon: typeof Droplet; label: string; tone: "primary" | "warning" | "info"; onClick: () => void }) {
  const map = {
  ...
  return (
    <button onClick={onClick} className={`rounded-3xl p-7 shadow-lg active:scale-[0.99] transition flex flex-col items-center gap-3 ${map[tone]}`}>
